...
metric <- "bray"
ns <- c(17, 23, 28, 33, 38, 43, 48)

df.all.env.sim.2 <- as.data.frame(setNames(replicate(length(ns), numeric(0)), sprintf("%.f", ns)))
df.all.geo.sim.2 <- as.data.frame(setNames(replicate(length(ns), numeric(0)), sprintf("%.f", ns)))
df.act.env.sim.2 <- as.data.frame(setNames(replicate(length(ns), numeric(0)), sprintf("%.f", ns)))
df.act.geo.sim.2 <- as.data.frame(setNames(replicate(length(ns), numeric(0)), sprintf("%.f", ns)))
...
for (i in c(1:100)){
  ...
  for (n in ns){
    rows <- sample(c(1:49), n, replace=FALSE)
    env1 <- env[rows, ]
    env.dat <- env1
    ...
    env.dat <- as.matrix(subset(env1, select = Elevation:TP))
  ...
  df.all.env.sim.2 <- rbind(df.all.env.sim.2, all.env.sim)
  df.all.geo.sim.2 <- rbind(df.all.geo.sim.2, all.geo.sim)
  df.act.env.sim.2 <- rbind(df.act.env.sim.2, act.env.sim)
  df.act.geo.sim.2 <- rbind(df.act.geo.sim.2, act.geo.sim)
}
```
...
```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE, fig.width=6, fig.height=6}
plot.new()
file <- paste("~/GitHub/DormancyDecay/figs/FromSites/SampSize_effect.png", sep="")
png(filename=file, width=8, height=4, units="in", res=600, pointsize=10)

par(mfrow=c(1, 2), mar = c(5,5,3,1), oma =c(0,0,0,0))

xs <- c(17, 23, 28, 33, 38, 43, 48)
xlabel <- "Number of sites"

# Make Plot for Environmental DDRs
avg1 <- colMeans(df.all.env.sim.2)
sem1 <- sapply(df.all.env.sim.2, function(x)sd(x)/sqrt(length(x)))

avg2 <- colMeans(df.act.env.sim.2)
sem2 <- sapply(df.act.env.sim.2, function(x)sd(x)/sqrt(length(x)))
...
plot(xs, avg1, ylab="DDR slope", xlab=xlabel,
  col = 'blue', cex=1, cex.lab=1.5, cex.axis = 1, pch=16, ylim=c(ymin, ymax))
text(44, -0.23, labels = 'total', cex = 1.5, col = 'blue')
arrows(xs, avg1 - sem1, xs, avg1 + sem1, length = 0.05, angle=90, code=3, col='blue')

points(xs, avg2, ylab="DDR slope", xlab=xlabel,
  col = 'red', cex=1, cex.lab=1.5, cex.axis = 1, pch=16, ylim=c(ymin, ymax))
text(44, -0.29, labels = 'active', cex = 1.5, col = 'red')
...
avg1 <- colMeans(df.all.geo.sim.2)
sem1 <- sapply(df.all.geo.sim.2, function(x)sd(x)/sqrt(length(x)))
avg2 <- colMeans(df.act.geo.sim.2)
sem2 <- sapply(df.act.geo.sim.2, function(x)sd(x)/sqrt(length(x)))
...
plot(xs, avg1, ylab="DDR slope", xlab=xlabel,
  col = 'blue', cex=1, cex.lab=1.5, cex.axis = 1, pch=16,  ylim=c(ymin, ymax))
text(44, -0.14, labels = 'total', cex = 1.5, col = 'blue')
arrows(xs, avg1 - sem1, xs, avg1 + sem1, length = 0.05, angle=90, code=3, col='blue')

points(xs, avg2, ylab="DDR slope", xlab=xlabel,
  col = 'red', cex=1, cex.lab=1.5, cex.axis = 1, pch=16, ylim=c(ymin, ymax))
text(44, -0.17, labels = 'active', cex = 1.5, col = 'red')
arrows(xs, avg2 - sem2, xs, avg2 + sem2, length = 0.05, angle=90, code=3, col='red')
