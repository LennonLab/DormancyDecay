...
env <- read.table("~/GitHub/DistDecay/data/Environmental/20130801_INPondData.csv", sep = ",", header = TRUE)
...
env.dat <- as.matrix(subset(env, select = c("canopy", "DO", "ORP", "pH", "Temp", "DOC", "DON", "TDS", "Color", "chla", "TP", "Depth", "Salinity", "SpC")))
...
for (i in c(1:1000)){
  ...
  for (n in ns){
    rows <- sample(c(1:49), 13, replace=FALSE)
    env.dat <- env[rows, ]
...
```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE, fig.width=6, fig.height=6}
file <- paste("~/GitHub/DistDecay/figs/Fig2-DDR-v2x2.png", sep="")
png(filename=file, width=5, height=5, units="in", res=600, pointsize=10)
...
#legend(2.0, -0.004, legend=c("total", "active"),
...
points(xs, colMeans(df.act.env.sor), ylab="DDR slope", xlab=xlabel,
  col = 'red', cex=1, cex.lab=1.2, cex.axis = 1, pch=16)
arrows(xs, avg - sem, xs, avg + sem, length = 0.05, angle=90, code=3, col='red')
legend(1.1, -0.14, legend=c("total", "active"),
       col=c("blue", "red"), pch=16, cex=1,
       box.lty=0)
...
points(xs, colMeans(df.act.geo.sor), ylab="DDR slope", xlab=xlabel,
  col = 'red', cex=1, cex.lab=1.2, cex.axis = 1, pch=16)
arrows(xs, avg - sem, xs, avg + sem, length = 0.05, angle=90, code=3, col='red')
legend(1.1, -0.025, legend=c("total", "active"),
       col=c("blue", "red"), pch=16, cex=1,
       box.lty=0)
...
```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE, fig.width=6, fig.height=6}
file <- paste("~/GitHub/DistDecay/figs/Fig2-DDR-v1x2.png", sep="")
png(filename=file, width=5, height=2.5, units="in", res=600, pointsize=10)

par(mfrow=c(1, 2), mar = c(5,5,3,1), oma =c(0,0,0,0))

xs <- log10(ns-1)
xlabel <- "log10(sampling depth)"

# Make Plot for Environmental DDRs
avg <- colMeans(df.all.env.bray)
sem <- sapply(df.all.env.bray, function(x)sd(x)/sqrt(length(x)))
plot(xs, colMeans(df.all.env.bray), ylab="DDR slope", xlab=xlabel,
  col = 'blue', cex=1, cex.lab=1.2, cex.axis = 1, pch=16, ylim=c(-0.65, -0.35))
arrows(xs, avg - sem, xs, avg + sem, length = 0.05, angle=90, code=3, col='blue')
...
sem <- sapply(df.all.geo.bray, function(x)sd(x)/sqrt(length(x)))
plot(xs, colMeans(df.all.geo.bray), ylab="DDR slope", xlab=xlabel,
  col = 'blue', cex=1, cex.lab=1.2, cex.axis = 1, pch=16,  ylim=c(-0.11, -0.06))
arrows(xs, avg - sem, xs, avg + sem, length = 0.05, angle=90, code=3, col='blue')
